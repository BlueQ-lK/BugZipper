      ...
      <div class="dashboard">
        <div class="box-1">
            ...
            <div class="port-sacn">
              <div class="first-line">
                <div class="input-field">
                  <input type="text" required v-model="insertnumber.ip_addr" :style="'border:' + scan_message.border"
                    placeholder="Enter IP Address" />
                </div>
                <div class="port-input-box">
                  <input type="text" v-model="insertnumber.port" placeholder="Port Number"
                    title=" Ex: 22 | 1-65535 | 53,111,137">
                </div>
              </div>
                ...
                <div style="margin-top: 1rem;">
                  <div class="save-btn">
                    <button @click="insertnumber.save = !insertnumber.save"
                      :style="{ backgroundColor: insertnumber.save ? 'green' : '' }">SAVE</button>
                    <input type="text" placeholder="New Scan" v-model="insertnumber.saveName" v-if="insertnumber.save">
                    <input type="text" placeholder="New Scan" v-model="insertnumber.saveName" v-else readonly
                      style="opacity: 0.3;">
                    <div class="aggscan">
                      <button @click="insertnumber.aggscan = !insertnumber.aggscan"
                        :style="{ backgroundColor: insertnumber.aggscan ? 'green' : '' }">Deep<span
                          style="color: red;">|</span>scan</button>
                    </div>
                  </div>
          ...
          <div class="fa-text-middle">
            <span :style="'color:' + scan_message.color" style="position: absolute; top: -0.6rem;">{{
              scan_message.rmessage }}</span>
          </div>
          <!-- card-2 -->
          <div class="card-2">
            ...
            <div class="result" style="background-color: #1c1c1c;">
              <div class="welc-txt animation-1">
                <p>
                  ...
                  ██████╦╝╚██████╔╝╚██████╔╝███████╗██║██║░░░░░██║░░░░░███████╗██║░░██║
                  ╚═════╝░░╚═════╝░░╚═════╝░╚══════╝╚═╝╚═╝░░░░░╚═╝░░░░░╚══════╝╚═╝░░╚═╝ V 0.1
                  <span> █ Author: BlueQ-lK █ Started On: 10 Dec 2023 █ </span>
                </p>
                <br>
                <hr>
              </div>
              ...
              <div class="terminalText" v-if="isButtonClicked">
                ...
                <div v-for="(scan, index) in scan_info.port_info" :key="index">
                  <p v-if="scan.state == 'open'">Discovered open port {{ scan.portid + "/" + scan.protocol }} on {{
                    insertnumber.ip_addr }}</p>
                  ...
                  <div class="de-port-result" v-for="(scan, index) in scan_info.port_info" :key="index">
                    <dl v-if="scan.portid == showResultterminalValue[1]">
                      ...
                      <dd>
                        ...
                        <div v-if="scan_info.script_info.some(scanScript => scanScript.port_id === scan.id)"
